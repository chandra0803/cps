...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lxslt="http://xml.apache.org/xslt" version="1.0">
  ...
  <xsl:import href="inf/navigation.xsl"/>
...
<xsl:template match="Screen">
  ...
  <body bgcolor="#FFFFFF" marginwidth="0" marginheight="0" leftmargin="0" topmargin="0" rightmargin="0" bottommargin="0" onUnload="localPageUnloaded();" onLoad="pageLoaded();">
  ...
  <xsl:call-template name="Banner">
    <xsl:with-param name="uid" select="Model/Session/UserProfile/@uid"/>
    <xsl:with-param name="imgBaseUrl" select="$imgBaseUrl"/>
    <xsl:with-param name="relUrl" select="$relUrl"/>
  </xsl:call-template>

  <xsl:apply-templates select="Model/Session/UserTabs">
    <xsl:with-param name="imgBaseUrl" select="$imgBaseUrl"/>
    <xsl:with-param name="relUrl" select="$relUrl"/>
  </xsl:apply-templates> 
  ...
  <TABLE width="100%" cellspacing="0" cellpadding="0" border="0">
    <TR>
      <TD WIDTH="195" height="99%" VALIGN="top" ALIGN="right" bgcolor="#7B7B7B">
        <TABLE width="100%" cellspacing="0" cellpadding="0" border="0">
          <TR>
            <TD WIDTH="195" height="1%" VALIGN="top" ALIGN="right" bgcolor="#7B7B7B">
              <Img src="{$imgBaseUrl}/clear.gif" width="190" height="1"/>
            </TD>
          </TR>
          <TR>
            <TD>
              <xsl:apply-templates select="Model/Session/UserNavigation">
                <xsl:with-param name="imgBaseUrl" select="$imgBaseUrl"/>
                <xsl:with-param name="relUrl" select="$relUrl"/>
              </xsl:apply-templates> 
            </TD>
      ...
      <TD WIDTH="100%" VALIGN="top" ALIGN="left" rowspan="3">
    ...
    <TR>
      <TD WIDTH="195" VALIGN="bottom" ALIGN="right" bgcolor="#7B7B7B">
      <img src="{$imgBaseUrl}/screen/bottomCorner.gif"/>
      </TD>
    </TR>
    <TR>
      <TD WIDTH="195" VALIGN="center" ALIGN="left" bgcolor="#535252">
        <TABLE width="100%" cellspacing="5" cellpadding="10" border="0">
        <TR>
        <TD>
          <FONT class="v7w">&#169;&#160;2004 Delphi Corporation.<BR/>All rights reserved.</FONT>
        </TD>
        </TR>
        </TABLE>
      </TD>
    ...
    <TR>
      <TD colspan="2" bgcolor="#FE0000" height="2"><img alt="" height="2" width="1" src="{$imgBaseUrl}/redline_pixel.gif"/></TD>
    </TR>
  </TABLE>
  </form>
  </body>
